...
describe('Endpoints PUT de Usuarios', () => {
  test('Debe actualizar un usuario existente', async () => {
  const updatedData = {
  nombre: 'Gaston',
  email: 'noemail@prueba.com',
  };
  
  const response = await api.put(`/api/usuarios/1`).send(updatedData);
  ...
  test('Debe actualizar un usuario existente x2, se usa para volver al id 1 al estado original', async () => {
    const updatedData = {
    nombre: 'Enzo',
    email: 'enzofernandezunahur@gmail.com',
    };
  ...
  const response = await api.put(`/api/usuarios/999`).send(updatedData); //id que no tenemos, habria que modificarlo si llegamos a tener 999 usuarios
  
  expect(response.status).toBe(404);
  expect(response.body.message).toBe('Usuario con ID 999 no encontrado');
  });
  
  test('Debe retornar 400 si no se proporcionan datos para actualizar', async () => {
  const response = await api.put(`/api/usuarios/1`).send({});
  
  expect(response.status).toBe(400);
  expect(response.body.message).toBe('No se proporcionaron datos para actualizar');
  });
  });

describe('Endpoints Delete de Usuarios', () => {
  test('Debe eliminar un usuario existente', async () => {
    const response = await api.delete(`/api/usuarios/16`); // Crear un usuario de prueba con el id necesario
    ...
    test('Debe retornar 404 si el usuario no existe', async () => {
    const response = await api.delete(`/api/usuarios/999`); // Asumiendo que el usuario con ID 999 no existe
