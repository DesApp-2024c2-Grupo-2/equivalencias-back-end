...
describe('Endpoints GET de Equivalencias', () => {
  ...
  test('Debe retornar un listado de equivalencias, materias solicitadas y datos de usuarios', async () => {
    ...
    expect(response.body[0].instituto).toBe("Untref");
    expect(response.body[0].estado).toBe("pendiente");
    expect(response.body[0].observaciones).toBe("falta analitico");
  ...
  test('Debe retornar una equivalencia dado su id con materias solicitadas y datos de usuarios', async () => {
    ...
    //Verifica datos de la equivalencia
    expect(response.body.id).toBe(1);
    expect(response.body.instituto).toBe("Untref");
    expect(response.body.estado).toBe("pendiente");
    expect(response.body.observaciones).toBe("falta analitico");
  ...
  test('Debe retornar una lista con todas las equivalencias', async () => {
    ...
    expect(response.body.data[0].estado).toBe("pendiente");
    expect(response.body.data[0].instituto).toBe("Untref");
    expect(response.body.data[0].observaciones).toBe("falta analitico");
  });
  ...
  test('Debe retornar una equivalencia dado un id', async () => {
    ...
    expect(response.body.data.instituto).toBe("Untref");
    expect(response.body.data.observaciones).toBe("falta analitico");
  });
