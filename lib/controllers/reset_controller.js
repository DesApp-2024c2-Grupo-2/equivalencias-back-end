...
export const updatePassword = async (req, res) => {
    ...
    const listaPedidoReset = await Reset.findAll({ where: { hash: usuarioHash } });
    const cambioEstados = await Reset.update({ estado: false }, { where: { dni: usuarioDni } });
    const cambiarPassword = await Usuario.update({ password: usuarioNUevaPassword }, { where: { dni: usuarioDni } })
    const pedidoReset = listaPedidoReset[listaPedidoReset.length - 1];
    const usuarioResetPassword = await Usuario.findOne({ where: { dni: usuarioDni } });

    const updateReset = (pedido) => {
        console.log(usuarioDni);
        if (usuarioDni == usuarioResetPassword.dni && usuarioDni == pedido.dni && usuarioHash == pedido.hash) {
            cambiarPassword
            cambioEstados
            return {
                status: 200,
